...
export class ProductComponent extends RouteObserverService implements OnInit, OnDestroy {
  ...
  private _setIsHome(){
    if(this._router.url == "/crm/product"){
      this.isHome = true;
      this._initDashboard();
    }else{
  ...
  private _initDashboard(){
    ...
    this._subs.id('dash').sink = this._crm.product_item(_filters, "post", null, "0").subscribe({
      next : (value) => {
    ...
    this.statistics = [
      {
        ...
        no_data    : {
                        ...
                        callback    : (p) => {},
                        api: (rawValue: any) => {
                          return this._crm.product_item(rawValue);
                        },
