...
@Component({
  selector: 'eg-payment-reminder-detail',
  templateUrl: './payment-reminder-detail.component.html',
  styleUrls: ['./payment-reminder-detail.component.scss']
})
export class PaymentReminderDetailComponent {
  ...
  ngOnInit(){
    ...
    this.fp = Sales.getPaymentRemindersFormProperties();
    ...
    this.config = {
      ...
      header: Core.Localize('payment_reminder', {count: 1}),
      params: {...this.params, limit: 100},
      itemId        : _itemId,
      detailsApi$     : (param, method, nextUrl)   => this._crm.sales_payment_reminders(param, method, nextUrl)/* .pipe(tap(res => {
