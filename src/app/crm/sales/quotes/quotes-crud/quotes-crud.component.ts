...
import { ProjectDetailComponent } from 'src/app/crm/project/project-detail/project-detail.component';
import { TitleCasePipe } from '@angular/common';

@Component({
  selector: 'eg-quotes-crud',
  templateUrl: './quotes-crud.component.html',
  styleUrls: ['./quotes-crud.component.scss']
})
export class QuotesCrudComponent extends RouteObserverService implements OnInit, OnDestroy {
  ...
  public api                : any = (p?: any, method?: apiMethod, nextUrl?) => this._crm.sales_quotes(p, method, nextUrl);
  public itemId             : any;
  ...
  constructor(private _wr                     : WrapperService,
    ...
    private _initDetailsContainer(itemId?){
      ...
      let _enabled_status = ['draft', 'confirmed']
      this.config = {
        showNavbar        : showBars,
          ...
          items  : [
            {
              ...
              subTitle: Core.Localize('basicInfoOfTheItem', {item: Core.Localize('quote')}),
            ...
            },
            {
              ...
              subTitle: Core.Localize('checkAllTheItems', {item: Core.Localize('quote')}),
            ...
            },
            {
              ...
              isDisabled : that._isDisabled,
              icon: "fa-solid fa-file-contract",
              subTitle: Core.Localize('conditionOfThisItem', {item: Core.Localize('quote')}),
...

            {
              ...
              subTitle: Core.Localize('dContainerSubHeader', {header: Core.Localize('quote')}),
        ...
        actionItems : [
          ...
          {
            label: Core.Localize('email_item', {item: Core.Localize('quote')}),
            icon: "fa-solid fa-envelope",
            styleClass : "text-lg",
            command : (event) => {
              const _dialogConf = <DynamicDialogConfig<IEmailRendererData>>{
                ...
                header: Core.Localize('email_item', {item: Core.Localize('quote')}),
                showHeader: true,
                maximizable: true,
                width: '85vw',
                height: '90vh',
          ...
          {
            label: Core.Localize('print', {item: Core.Localize('quote')}),
          ...
          {
            label: Core.Localize('duplicate_item', {item: Core.Localize('quote')}),
            ...
            command : (event) => {
              const _dialogConf = <DynamicDialogConfig<IPositionPreviewData>>{
                data: {
                  ...
                  template_type: 'quotes'/* ,
                  api: (p, m, n) => this._crm.sales_positions(p, m, n), */
                },
    ...
    private _setActionItems(status, data){
      ...
      switch(_status){
        case "draft":
          ...
          //set quote
          this.config.actionItems[0].label   = Core.Localize('setStatusTo', {status: 'Pending'})
        ...
        case "pending":
          this.config.workflowItems = this.workflow_items;
          //set quote
          this.config.actionItems[0].label   = Core.Localize('setStatusTo', {status: 'Draft'})
        ...
        case "confirmed":
          ...
          //set quote
          this.config.actionItems[0].label    = Core.Localize('edit', {item: Core.Localize('quote')})
          this.config.actionItems[0].icon     = 'fa-solid fa-pencil'
          //create order
      ...
      function hasProject(data){
        if(data != null){
          return `${Core.Localize('open', {item: data.name})}`;
        }else{
          return Core.Localize('create');
        }
      }
    }
