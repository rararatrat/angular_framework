...
@Component({
  selector: 'eg-comments',
  templateUrl: './comments.component.html',
  styleUrls: ['./comments.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class CommentsComponent extends GridListClass implements OnInit, AfterViewInit, OnDestroy {
  ...
  private _initFormProperty(){
    let temp : IAddComponentData = {
      ...
      includedFields : ['name', 'user', 'priority', 'status', 'project', 'files']
    }
    return temp;
  }
  ...
  private _initDetailsContainer(data) {
    this.detailsContainer = {
      ...
      detailsApi$    : (param, method, nextUrl)   => {
        return this.config?.api$(param, method, nextUrl);
      },
      hasUpdates    : false,
      hasComments   : false,
      hasLogs       : false,
      formProperty  : this._initFormProperty()
    };
    this.detailsConfig = new DetailsConfig({config: this.detailsContainer, fb: this._fb, wr: this._wr, cdr: this._cdr});
